...
import static com.comcast.viper.common.NTPValidator.stopValidator;
 ...
 * @author ctsuser1
 */
public class PTSValidator extends Thread {
    ...
    public void run(){
        ...
        DefaultTableModel table = (DefaultTableModel) ebpa.jTable2.getModel();  
        ...
        for(;;){
           ...
           // get common minimumarray size
            currRow = ebprList.get(0).fragPTS.size();
            for (int l = 0 ; l < ebprList.size();l++ ){
                fragSize = ebprList.get(l).fragPTS.size();
                if (l==0){
                    currRow = fragSize;                
            ...
            if (preRow < currRow){
                ...
                for (int i = 0 ; i < ebprList.size();i++ ){
                    ...
                    table.setValueAt(ebprList.get(i).fragPTS.get(currRow-1),currRow-1, colNum); 
                } */ 
                for (int i = 0 ; i < ebprList.size();i++ ){
                    colNum = getColumnByName(table, ebprList.get(i).port);
                    data[colNum]  = ebprList.get(i).fragPTS.get(currRow-1);
                }
                table.addRow(data);                
            } 
            // delay checks 
